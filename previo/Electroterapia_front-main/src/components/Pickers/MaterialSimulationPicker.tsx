...
import "./MaterialSimulationPicker.css";
...
const MaterialSimulationPicker: React.FC<{
  electrodo: Electrodo | null;
  setElectrodo: React.Dispatch<React.SetStateAction<Electrodo | null>>;
  elecType: TipoElectrodo;
  canal: 1 | 2;
  ...
  electrodo, setElectrodo, elecType, canal,
  label1, label2,
    ...
    <IonItem className="material-picker" lines="full">
      <IonSelect
        ...
        onIonChange={(e) => {
          ...
          setElectrodo((prev: Electrodo | null) => {
            ...
            return new Electrodo(undefined, undefined, elecType, undefined, nuevoMaterial, canal);
          });
        }}        
        color={'negro'}
      >
      ...
      {electrodo?.material != null &&
        <IonSelect
          ...
          disabled={disabled2}
          onIonChange={(e) => setElectrodo((prev: Electrodo | null) => 
            prev == null ? null
            : prev?.material?.type == "aguja" 
              ? new Electrodo(prev.x, prev.y, prev.type, prev.color, new MaterialAguja(e.detail.value), canal)
            : prev?.material?.type == "caucho" 
              ? new Electrodo(prev.x, prev.y, prev.type, prev.color, new MaterialCaucho(e.detail.value), canal)
            : prev?.material?.type == "adhesivo" 
              ? new Electrodo(prev.x, prev.y, prev.type, prev.color, new MaterialAdhesivo(e.detail.value), canal)
            : prev.setToNull()
          )}
          color={'negro'}
        >
...
export default MaterialSimulationPicker;
