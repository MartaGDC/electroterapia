...
export const UserListProvider = (props: any) => {
  // Almacena el valore de todos los usuarios
  const [allUsers, setAllUsers] = useState<User[]>([]);
  
  // Almacena la lista de usuarios visibles dado el input actual en la barra de
  // búsqueda de usuarios
  const [users, setUsers] = useState<User[]>([]);

  // Array de longitud = allUsers.length. Cada componente se corresponde con cada
  // usuario de allUsers, si la componente es false no está seleccionado, si es
  // true, si lo está
  const [selected, setSelected] = useState<SelectedUsersMap>({});
  
  // Número de profesores en la lista. Sirve para controlar el selectAll de modo
  // que no seleccione a los profesores
